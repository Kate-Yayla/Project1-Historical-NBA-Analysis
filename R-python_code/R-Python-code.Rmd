...
subset2py_no_outlier=subset2py2[~((subset2py2<(Q1-1.5*IQR)) | (subset2py2>(Q3+1.5*IQR)))]
...
for index in range(0,percentile_df.shape[0]):
  row_sum = sum(list(percentile_df.iloc[index])[0:-1])
  row_main_perc = (row_sum/(percentile_df.shape[1]-1))
  main_perc_list.append(row_main_perc)
  
  
percentile_df['Main Percent'] = main_perc_list
percentile_df
